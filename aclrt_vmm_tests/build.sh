...
jobs=8
build_type=Release
ascend_root="${ASCEND_ROOT:-${ASCEND_HOME_PATH:-}}"
...
for executable in "${executables[@]}"; do
  source_path="${build_dir}/${executable}"
  if [[ ! -x "${source_path}" ]]; then
    echo "Built executable was not found: ${source_path}" >&2
    exit 1
  fi
  install -m 0755 "${source_path}" "${output_dir}/bin/${executable}"
done

echo "Build output: ${output_dir}/bin"
