...
std::string FormatOptions(const Options& options)
{
    ...
    out << "  pid_validation=" << (options.disable_pid_validation ? "disabled" : "enabled") << "\n";
    out << "  requested_share_api=" << (options.force_v1 ? "V1" : "V2 if available") << "\n";
    out << "  share_type=" << (options.share_kind == ShareKind::Fabric ? "fabric" : "default")
        << "\n";
    out << "  build_has_v2_share_api=" << (ACLTEST_HAS_V2_SHARE_API ? "yes" : "no") << "\n";
    out << "  build_has_fabric_share_type=" << (ACLTEST_HAS_FABRIC_SHARE_TYPE ? "yes" : "no");
    return out.str();
}
...
std::string GetSocName()
{
    ...
    if (soc_name == nullptr || soc_name[0] == '\0') { return "unknown"; }
    return soc_name;
}
...
std::string DetectAscendFamily(const std::string& soc_name)
{
    ...
    if (Contains(lower, "a3") || Contains(lower, "910_93") || Contains(lower, "910_95")) {
        return "Ascend A3";
    }
    if (Contains(lower, "a2") || Contains(lower, "910b")) { return "Ascend A2"; }
    if (Contains(lower, "a5") || Contains(lower, "950") || Contains(lower, "cloud_v5") ||
        Contains(lower, "cloudv5")) {
        return "Ascend A5";
    }
    return "unknown";
}
...
std::string GetCannVersion()
{
    ...
    auto try_get_version = [](char* pkg_name) -> std::string {
        ...
        if (ret == ACL_SUCCESS && version_str[0] != '\0') { return version_str; }
        return {};
    };
    ...
    const aclError ret = aclrtGetVersion(&major_version, &minor_version, &patch_version);
    if (ret == ACL_SUCCESS) {
        std::ostringstream out;
...
bool ParseArgs(int argc, char** argv, Options* options)
{
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (value == nullptr || !ParseInt(value, &options->device)) { return false; }
        } else if (arg == "--host-numa") {
            const char* value = require_value("--host-numa");
            if (value == nullptr || !ParseInt(value, &options->host_numa)) { return false; }
        } else if (arg == "--size") {
            const char* value = require_value("--size");
            if (value == nullptr || !ParseSize(value, &options->requested_size)) { return false; }
...
bool ValidateDevice(const Options& options)
{
    uint32_t device_count = 0;
    {
        ...
        if (!LogAcl("aclrtGetDeviceCount", ret) || device_count == 0U) { return false; }
...
void PrintOptions(const Options& options) { std::cout << FormatOptions(options) << "\n"; }

std::string ResolveSiblingExecutable(const char* argv0, const char* sibling_name)
{
    char self_path[4096] = {};
    const ssize_t self_length = readlink("/proc/self/exe", self_path, sizeof(self_path) - 1);
    const std::string executable = self_length > 0
                                       ? std::string(self_path, static_cast<size_t>(self_length))
                                       : (argv0 == nullptr ? std::string() : std::string(argv0));
    const size_t separator = executable.rfind('/');
    if (separator == std::string::npos) { return sibling_name; }
    return executable.substr(0, separator + 1) + sibling_name;
}

}  // namespace

int RunTestProgram(int argc, char** argv, const char* title, const TestCase* tests,
                   size_t test_count, StartupDisplayMode display_mode)
{
    Options options;
    ...
    options.ipc_child_executable = ResolveSiblingExecutable(argv[0], "aclrt_vmm_ipc_child");
    ...
    if (!ValidateDevice(options)) { return 1; }
