...
{
    switch (kind) {
        case EndpointKind::DeviceVa: return "device VA";
        case EndpointKind::HostVa: return "host VA";
        case EndpointKind::DeviceBuffer: return "device buffer";
        case EndpointKind::HostBuffer: return "host buffer";
    }
    return "unknown";
}

bool IsDeviceBuffer(EndpointKind kind) { return kind == EndpointKind::DeviceBuffer; }
...
bool AllocateMapping(const Options& options, EndpointKind kind, PhysicalMapping* mapping)
{
    const auto config =
        kind == EndpointKind::HostVa ? MakeHostConfig(options) : MakeDeviceConfig(options);
    size_t aligned_size = 0;
    return QueryAlignedSize(config, options.requested_size, &aligned_size) &&
           AllocateAndMapPhysical(config, aligned_size, mapping);
}

Endpoint ResolveEndpoint(EndpointKind kind, PhysicalMapping* mapping, DeviceBuffer* device_buffer,
...
std::string MemcpyLabel(const char* phase, const Endpoint& source, const Endpoint& destination)
{
    return "aclrtMemcpy(" + std::string(phase) + " " + source.name + " -> " + destination.name +
           ")";
}

bool RunDirection(const Options& options, EndpointKind source_kind, EndpointKind destination_kind,
                  PhysicalMapping* source_mapping, PhysicalMapping* destination_mapping,
                  DeviceBuffer* device_buffer, uint32_t seed)
{
    std::vector<uint8_t> expected = MakePattern(options.requested_size, seed);
    std::vector<uint8_t> actual(options.requested_size, 0);
    Endpoint source = ResolveEndpoint(source_kind, source_mapping, device_buffer, &expected);
    Endpoint destination =
        ResolveEndpoint(destination_kind, destination_mapping, device_buffer, &actual);
...
bool RunCase(const Options& options, const CopyCase& test_case)
{
    ...
    if (ok && (IsDeviceBuffer(test_case.source) || IsDeviceBuffer(test_case.destination))) {
        ok = device_buffer.Allocate(options.requested_size);
    }
    if (!ok) { return false; }

    ok = RunDirection(options, test_case.source, test_case.destination, &source_mapping,
                      &destination_mapping, &device_buffer, test_case.seed_base + 1U);
    if (ok) {
        ok = RunDirection(options, test_case.destination, test_case.source, &destination_mapping,
                          &source_mapping, &device_buffer, test_case.seed_base + 2U);
    }
    return ok;
}
...
constexpr CopyCase kCopyCases[] = {
    ...
    {"[single-process] device VA <-> host buffer",   EndpointKind::DeviceVa, EndpointKind::HostBuffer,
     0x1200U                                                                                                    },
    {"[single-process] device VA <-> device VA",     EndpointKind::DeviceVa, EndpointKind::DeviceVa,
     0x1300U                                                                                                    },
    {"[single-process] host VA <-> device buffer",   EndpointKind::HostVa,   EndpointKind::DeviceBuffer,
     0x1400U                                                                                                    },
    {"[single-process] host VA <-> host buffer",     EndpointKind::HostVa,   EndpointKind::HostBuffer,
     0x1500U                                                                                                    },
    {"[single-process] host VA <-> host VA",         EndpointKind::HostVa,   EndpointKind::HostVa,       0x1600U},
    {"[single-process] device VA <-> host VA",       EndpointKind::DeviceVa, EndpointKind::HostVa,
     0x1700U                                                                                                    },
};
...
{
    AclRuntime runtime;
    if (!runtime.Init() || !runtime.SetDevice(options.device)) { return false; }

    bool ok = true;
    for (const auto& test_case : kCopyCases) { ok = RunCase(options, test_case) && ok; }
    return ok;
}
