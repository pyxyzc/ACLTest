...
constexpr const char *kBatchReadFunction = "AclTestSdmaBatchRead";
constexpr const char *kBatchWriteFunction = "AclTestSdmaBatchWrite";
constexpr const char *kSyncContextFunction = "AclTestSyncTransferContext";
...
AicpuBatchLauncher::~AicpuBatchLauncher() {
  Shutdown();
}

void AicpuBatchLauncher::LoadKernel(const std::string &kernel_json) {
  if (kernel_json.empty()) {
    throw std::invalid_argument("AICPU kernel JSON path cannot be empty");
  }
  ...
  ACLTEST_CHECK_ACL(aclrtBinaryGetFunction(binary_, kBatchReadFunction, &batch_read_));
  ACLTEST_CHECK_ACL(aclrtBinaryGetFunction(binary_, kBatchWriteFunction, &batch_write_));
  ACLTEST_CHECK_ACL(aclrtBinaryGetFunction(binary_, kSyncContextFunction, &sync_context_));
}

void AicpuBatchLauncher::CreateExecutionResources() {
...
void AicpuBatchLauncher::Initialize(int32_t logic_device_id, const std::string &kernel_json) {
  if (initialized_ || logic_device_id < 0) {
    throw std::invalid_argument("AICPU launcher is already initialized or device id is invalid");
  }
...
void AicpuBatchLauncher::BuildRtsqParam(uint32_t task_count, AicpuKernelParam &param) {
  ...
  if (rtStreamGetCqid(reinterpret_cast<rtStream_t>(worker_stream_), &cq_id, &logic_cq_id) != RT_ERROR_NONE) {
...
void AicpuBatchLauncher::LaunchOneBatch(aclrtFuncHandle function, TransferDirection direction, size_t begin,
                                        size_t count, size_t descriptor_count, uint32_t timeout_ms,
                                        size_t status_index, size_t &tasks_since_notify) {
  ...
  void *device_args = static_cast<uint8_t *>(request_.kernel_args) + status_index * sizeof(AicpuKernelParam);
  ACLTEST_CHECK_ACL(
      aclrtMemcpy(device_args, sizeof(param), &param, sizeof(param), ACL_MEMCPY_HOST_TO_DEVICE));
  ACLTEST_CHECK_ACL(
      aclrtLaunchKernelV2(function, kKernelBlockDim, device_args, sizeof(param), nullptr, control_stream_));
...
void AicpuBatchLauncher::LaunchBatches(TransferDirection direction, size_t descriptor_count, uint32_t timeout_ms) {
  ...
  while (begin < descriptor_count) {
    const size_t count = std::min(static_cast<size_t>(kMaxDescriptorsPerLaunch), descriptor_count - begin);
    LaunchOneBatch(function, direction, begin, count, descriptor_count, timeout_ms, status_index,
                   tasks_since_notify);
    begin += count;
    ++status_index;
  }
...
void AicpuBatchLauncher::Submit(TransferDirection direction, const std::vector<AddressRange> &ranges,
                                uint32_t timeout_ms) {
  if (!initialized_ || request_active_) {
    throw std::logic_error("AICPU launcher is not initialized or already has an active request");
  }
  const std::vector<AicpuTransferDesc> descriptors = BuildDeviceDescriptors(direction, ranges);
  last_shape_ = ComputeSubmissionShape(descriptors.size());
  request_active_ = true;
  request_launched_ = false;
  request_.descriptor_bytes = descriptors.size() * sizeof(AicpuTransferDesc);
  request_.status_count = last_shape_.kernel_launch_count;
  request_.kernel_arg_count = last_shape_.kernel_launch_count;

  try {
    ACLTEST_CHECK_ACL(
        aclrtMalloc(&request_.descriptors, request_.descriptor_bytes, ACL_MEM_MALLOC_NORMAL_ONLY));
    ...
    ACLTEST_CHECK_ACL(aclrtMalloc(&request_.kernel_args,
                                  request_.kernel_arg_count * sizeof(AicpuKernelParam),
                                  ACL_MEM_MALLOC_NORMAL_ONLY));
    ...
    ACLTEST_CHECK_ACL(aclrtMemcpy(request_.descriptors, request_.descriptor_bytes, descriptors.data(),
                                  request_.descriptor_bytes, ACL_MEMCPY_HOST_TO_DEVICE));
...
void AicpuBatchLauncher::Synchronize(uint32_t timeout_ms) {
  if (!request_active_) {
    throw std::logic_error("no active AICPU request to synchronize");
  }
  const uint32_t effective_timeout = timeout_ms == 0U ? kDefaultTimeoutMs : timeout_ms;
  try {
    ACLTEST_CHECK_ACL(
        aclrtSynchronizeStreamWithTimeout(control_stream_, static_cast<int32_t>(effective_timeout)));
  } catch (...) {
    Abort();
    throw;
  }
...
void AicpuBatchLauncher::Finish() {
  if (!request_active_) {
    throw std::logic_error("no completed AICPU request to finish");
  }
  std::vector<uint32_t> statuses(request_.status_count, 0U);
  try {
    ACLTEST_CHECK_ACL(aclrtMemcpy(statuses.data(), statuses.size() * sizeof(uint32_t), request_.statuses,
                                  request_.status_count * sizeof(uint32_t), ACL_MEMCPY_DEVICE_TO_HOST));
  } catch (...) {
    ReleaseRequest();
    throw;
  }
  ...
  for (size_t index = 0U; index < statuses.size(); ++index) {
    if (statuses[index] != 0U) {
      throw std::runtime_error("AICPU SDMA launch status[" + std::to_string(index) + "]=" +
                               std::to_string(statuses[index]));
    }
  }
}

uint32_t AicpuBatchLauncher::SyncTransferContextOnce(uint32_t operation) {
  TransferContextSyncEntry entry{};
  entry.key = transfer_context_key_;
  entry.op = operation;
  ...
  try {
    ...
    ACLTEST_CHECK_ACL(aclrtMemcpy(device_entry, sizeof(entry), &entry, sizeof(entry), ACL_MEMCPY_HOST_TO_DEVICE));
    ...
    ACLTEST_CHECK_ACL(aclrtLaunchKernelV2(
        sync_context_, kKernelBlockDim, device_args, sizeof(param), nullptr, default_stream));
    ACLTEST_CHECK_ACL(
        aclrtSynchronizeStreamWithTimeout(default_stream, static_cast<int32_t>(kSyncContextTimeoutMs)));
    uint32_t state = kTransferContextDeleted;
    ACLTEST_CHECK_ACL(
        aclrtMemcpy(&state, sizeof(state), device_state, sizeof(state), ACL_MEMCPY_DEVICE_TO_HOST));
...
void AicpuBatchLauncher::SyncTransferContext(uint32_t operation, uint32_t expected_state) {
  const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(kDeleteRetryMs);
  for (;;) {
    ...
    if (operation != kTransferContextDelete || state != kTransferContextDeleting) {
      throw std::runtime_error("unexpected AICPU transfer context state " + std::to_string(state));
    }
    if (std::chrono::steady_clock::now() >= deadline) {
      throw std::runtime_error("timed out deleting AICPU transfer context");
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(kDeleteRetryIntervalMs));
  }
}

void AicpuBatchLauncher::ReleaseRequest() noexcept {
...
void AicpuBatchLauncher::DestroyExecutionResources(bool abort_streams) noexcept {
...
void AicpuBatchLauncher::Abort() noexcept {
  if (control_stream_ != nullptr) {
    LogCleanupAcl("aclrtStreamAbort(control)", aclrtStreamAbort(control_stream_));
  }
  if (worker_stream_ != nullptr) {
...
void AicpuBatchLauncher::Shutdown() noexcept {
  if (request_active_) {
    Abort();
