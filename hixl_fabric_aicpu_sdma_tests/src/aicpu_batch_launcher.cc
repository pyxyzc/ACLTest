...
AicpuBatchLauncher::~AicpuBatchLauncher() { Shutdown(); }
...
void AicpuBatchLauncher::Initialize(int32_t logic_device_id, const std::string &kernel_json)
{
    if (initialized_ || logic_device_id < 0) {
        throw std::invalid_argument(
            "AICPU launcher is already initialized or device id is invalid");
    }
...
void AicpuBatchLauncher::LaunchOneBatch(aclrtFuncHandle function, TransferDirection direction,
                                        ...
                                        size_t &tasks_since_notify)
{
    ...
    void *device_args =
        static_cast<uint8_t *>(request_.kernel_args) + status_index * sizeof(AicpuKernelParam);
    ACLTEST_CHECK_ACL(
...
void AicpuBatchLauncher::LaunchBatches(TransferDirection direction, size_t descriptor_count,
                                       uint32_t timeout_ms)
{
...
void AicpuBatchLauncher::Submit(TransferDirection direction,
                                const std::vector<AddressRange> &ranges, uint32_t timeout_ms)
{
    if (!initialized_ || request_active_) {
        throw std::logic_error(
            "AICPU launcher is not initialized or already has an active request");
    }
...
{
    if (!request_active_) { throw std::logic_error("no active AICPU request to synchronize"); }
    const uint32_t effective_timeout = timeout_ms == 0U ? kDefaultTimeoutMs : timeout_ms;
    try {
...
{
    if (!request_active_) { throw std::logic_error("no completed AICPU request to finish"); }
...
void AicpuBatchLauncher::SyncTransferContext(uint32_t operation, uint32_t expected_state)
{
    ...
    for (;;) {
        ...
        if (operation != kTransferContextDelete || state != kTransferContextDeleting) {
            throw std::runtime_error("unexpected AICPU transfer context state " +
                                     std::to_string(state));
        }
