...
uint64_t ParseMultiplier(char suffix) {
  switch (static_cast<char>(std::toupper(static_cast<unsigned char>(suffix)))) {
    ...
    default:
      throw std::invalid_argument("unsupported byte-size suffix");
  }
}
...
uint64_t ParseByteSize(const std::string &text) {
  ...
  if (value.empty()) {
    throw std::invalid_argument("byte size cannot be empty");
  }
  uint64_t multiplier = 1U;
  if (value.size() >= 3U && (value[value.size() - 2U] == 'i' || value[value.size() - 2U] == 'I') &&
  ...
  if (parsed != value.size() || number == 0U || number > std::numeric_limits<uint64_t>::max() / multiplier) {
...
std::vector<uint64_t> ParseByteSizeList(const std::string &text) {
  ...
  while (std::getline(stream, item, ',')) {
    values.push_back(ParseByteSize(item));
  }
  if (values.empty()) {
    throw std::invalid_argument("size list cannot be empty");
  }
  return values;
}
...
std::vector<size_t> ParseCountList(const std::string &text) {
  ...
  while (std::getline(stream, item, ',')) {
    ...
    if (item.empty() || parsed != item.size() || count == 0U || count > std::numeric_limits<size_t>::max()) {
      throw std::invalid_argument("invalid IO count: " + item);
    }
    values.push_back(static_cast<size_t>(count));
  }
  if (values.empty()) {
    throw std::invalid_argument("count list cannot be empty");
  }
  return values;
}
...
std::vector<AicpuTransferDesc> BuildDeviceDescriptors(TransferDirection direction,
  ...
  if (ranges.empty()) {
    throw std::invalid_argument("transfer range list cannot be empty");
  }
  std::vector<AicpuTransferDesc> descriptors;
...
SubmissionShape ComputeSubmissionShape(size_t descriptor_count) {
  ...
  shape.kernel_launch_count =
      (descriptor_count + static_cast<size_t>(kMaxDescriptorsPerLaunch) - 1U) / kMaxDescriptorsPerLaunch;
