...
private:
    ...
    static bool BuildNotifySqe(uint32_t notify_id, uint32_t task_id, const RtsqState &state,
                               A3NotifySqe &sqe);
    ...
    static bool AppendNotify(uint32_t notify_id, RtsqState &state, RtsqBatch &batch,
                             uint64_t deadline);
};
