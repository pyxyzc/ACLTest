...
drvError_t __attribute__((weak)) drvGetLocalDevIDByHostDevID(uint32_t host_dev_id,
...
void LogError(const char *message) { std::printf("[AclTest][AICPU][ERROR] %s\n", message); }

}  // namespace

uint32_t StarsSdma::Submit(const AicpuKernelParam &param, const AicpuTransferDesc *descriptors)
{
    if (!Validate(param, descriptors)) { return kFailure; }
    RtsqState state;
    ...
    if (!BuildDeadline(param.timeout_ms, deadline)) { return kFailure; }
    RtsqBatch batch;
    ...
    if (param.emit_notify_record != 0U) {
        ...
        notify_ok = AppendNotify(param.notify_id, state, batch, deadline) &&
                    Publish(state, batch, deadline);
...
{
    if (descriptors == nullptr || param.desc_count == 0U ||
        param.desc_count > kMaxDescriptorsPerLaunch || param.emit_notify_record > 1U ||
...
{
    if (drvGetLocalDevIDByHostDevID == nullptr) { return false; }
...
{
    if (halResourceIdRestore == nullptr) { return true; }
...
bool StarsSdma::LoadQueueState(RtsqState &state)
{
    ...
    return state.base_addr != 0U && state.depth > kMaxEntriesPerPublish &&
           state.head < state.depth && state.tail < state.depth;
}
...
uint64_t StarsSdma::MonotonicNs()
{
    ...
    return static_cast<uint64_t>(value.tv_sec) * kNsPerSecond +
...
LogicCqResult StarsSdma::ReceiveLogicCq(const RtsqState &state, uint8_t *reports, uint32_t &count)
{
    ...
    if (result == DRV_ERROR_WAIT_TIMEOUT ||
        (result == DRV_ERROR_NONE && info.report_cqe_num == 0U)) {
        count = 0U;
...
{
    if (halCqReportRecv == nullptr) { return true; }
    ...
    for (;;) {
        ...
        if (result == LogicCqResult::kEmpty) { return true; }
        ...
        for (uint32_t index = 0U; index < count; ++index) {
            ...
            if ((report->error_type & kStarsErrorMask) != 0U) {
                std::printf(
                    "[AclTest][AICPU][ERROR] abnormal CQE: stream=%u task=%u code=%u type=%u "
                    "sq=%u\n",
                    static_cast<uint32_t>(report->stream_id),
                    static_cast<uint32_t>(report->task_id), report->error_code,
                    static_cast<uint32_t>(report->error_type),
...
{
    if (count == 0U || count >= state.depth) { return false; }
...
{
    if (HasCapacity(state, count)) { return true; }
    ...
    if (!HasCapacity(state, count)) {
        std::printf(
            "[AclTest][AICPU][ERROR] worker RTSQ has no capacity: depth=%u head=%u tail=%u need=%u "
...
bool StarsSdma::CopyToRing(const RtsqState &state, const RtsqBatch &batch)
{
    ...
    std::memcpy(base + static_cast<size_t>(state.tail) * kA3RtsqEntryBytes, batch.entries,
                first_bytes);
    if (first_count < batch.count) {
        const size_t wrap_bytes =
            static_cast<size_t>(batch.count - first_count) * kA3RtsqEntryBytes;
...
bool StarsSdma::CommitTail(RtsqState &state, uint32_t new_tail)
{
    ...
    if (halSqCqConfig(state.device_id, &config) != DRV_ERROR_NONE) { return false; }
    state.tail = new_tail;
    return true;
}
...
{
    if (batch.count == 0U) { return true; }
    if (!EnsureCapacity(state, batch.count, deadline)) { return false; }
    const uint32_t new_tail = (state.tail + batch.count) % state.depth;
    if (!CopyToRing(state, batch) || !CommitTail(state, new_tail)) { return false; }
    batch.count = 0U;
    return true;
}
...
bool StarsSdma::AppendNotify(uint32_t notify_id, RtsqState &state, RtsqBatch &batch,
                             uint64_t deadline)
{
    if (batch.count == kMaxEntriesPerPublish && !Publish(state, batch, deadline)) { return false; }
    if (!BuildNotifySqe(notify_id, state.next_task_id++, state,
                        batch.entries[batch.count].notify)) {
        return false;
    }
    ++batch.count;
    return true;
