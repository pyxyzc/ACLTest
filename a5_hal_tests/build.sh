...
install -m 0755 "${build_dir}/acl_a5_hal_test" "${output_dir}/bin/acl_a5_hal_test"
install -m 0444 "${project_dir}/device/libacltest_a5_hal_test.json" \
  "${output_dir}/opp/built-in/op_impl/aicpu/config/libacltest_a5_hal_test.json"

mapfile -t archives < <(find "${build_dir}" -type f -name 'acltest-a5-hal-test-compat.tar.gz' | sort)
if [[ ${#archives[@]} -eq 0 ]]; then
  echo "Built AICPU archive acltest-a5-hal-test-compat.tar.gz was not found under ${build_dir}" >&2
  exit 1
fi
install -m 0444 "${archives[${#archives[@]}-1]}" \
  "${output_dir}/opp/built-in/op_impl/aicpu/kernel/acltest-a5-hal-test-compat.tar.gz"
