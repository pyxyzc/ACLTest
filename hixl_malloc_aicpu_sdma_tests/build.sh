...
install -m 0755 "${build_dir}/acl_malloc_copy_bench" \
  "${output_dir}/bin/acl_malloc_copy_bench"
install -m 0444 "${project_dir}/device/libacltest_malloc_sdma_kernel.json" \
  "${output_dir}/opp/built-in/op_impl/aicpu/config/libacltest_malloc_sdma_kernel.json"

mapfile -t archives < <(find "${build_dir}" -type f -name 'acltest-malloc-sdma-compat.tar.gz' | sort)
if [[ ${#archives[@]} -eq 0 ]]; then
  echo "Built AICPU archive acltest-malloc-sdma-compat.tar.gz was not found under ${build_dir}" >&2
  exit 1
fi
install -m 0444 "${archives[${#archives[@]}-1]}" \
  "${output_dir}/opp/built-in/op_impl/aicpu/kernel/acltest-malloc-sdma-compat.tar.gz"
