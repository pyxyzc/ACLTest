...
bool IsA3Soc(const char *name) {
  ...
  return std::any_of(kNames.begin(), kNames.end(), [name](const char *candidate) {
    return std::string(name) == candidate;
  });
}

std::string KernelJsonPath(const std::string &root) {
  return root + "/opp/built-in/op_impl/aicpu/config/libacltest_malloc_sdma_kernel.json";
}
...
std::string TakeValue(int argc, char **argv, int &index, const std::string &argument) {
  ...
  if (equals != std::string::npos) {
    return argument.substr(equals + 1U);
  }
  if (index + 1 >= argc) {
...
std::vector<TransferDirection> ParseDirections(const std::string &value) {
  if (value == "d2h") {
    return {TransferDirection::kDeviceToHost};
  }
  if (value == "h2d") {
    return {TransferDirection::kHostToDevice};
  }
  if (value == "both") {
...
Options ParseOptions(int argc, char **argv) {
  ...
  for (int index = 1; index < argc; ++index) {
    ...
    if (name == "--device") {
      ...
      options.warmup = static_cast<size_t>(std::stoull(TakeValue(argc, argv, index, argument)));
...
Timing RunMemcpyLoop(const std::vector<AddressRange> &ranges, TransferDirection direction,
  ...
  for (const AddressRange &range : ranges) {
    void *destination = reinterpret_cast<void *>(direction == TransferDirection::kDeviceToHost
                                                     ? range.local_addr
                                                     : range.device_addr);
    const void *source = reinterpret_cast<const void *>(direction == TransferDirection::kDeviceToHost
                                                            ? range.device_addr
                                                            : range.local_addr);
...
std::vector<uint8_t> MakePattern(size_t bytes, TransferDirection direction, uint64_t io_size, size_t count) {
...
void CheckEqual(const uint8_t *actual, const std::vector<uint8_t> &expected, const std::string &method) {
  for (size_t index = 0U; index < expected.size(); ++index) {
    if (actual[index] != expected[index]) {
      throw std::runtime_error(method + " correctness failure at byte " + std::to_string(index) +
...
void VerifyMethods(MallocMemoryPair &memory, const std::vector<AddressRange> &ranges,
  ...
  if (direction == TransferDirection::kDeviceToHost) {
    ...
    ACLTEST_CHECK_ACL(aclrtMemcpy(memory.device(), total_bytes, host, total_bytes, ACL_MEMCPY_HOST_TO_DEVICE));
  ...
  ACLTEST_CHECK_ACL(
      aclrtMemcpy(actual.data(), total_bytes, memory.device(), total_bytes, ACL_MEMCPY_DEVICE_TO_HOST));
...
ResultRow RunCase(MallocMemoryPair &memory, BaselineStream &baseline, AicpuBatchLauncher &launcher,
                  const Options &options, TransferDirection direction, uint64_t io_size, size_t io_count) {
  ...
  VerifyMethods(memory, ranges, direction, io_size, io_count, baseline, launcher, options.timeout_ms);
  ...
  for (size_t index = 0U; index < iterations; ++index) {
    if ((index & 1U) == 0U) {
      memcpy_timings.push_back(RunMemcpyLoop(ranges, direction, baseline.get(), options.timeout_ms));
...
void PrintResults(const std::vector<ResultRow> &results) {
  ...
  std::cout << std::left << std::setw(4) << "dir" << std::setw(7) << "size" << std::right
            ...
            << std::setw(10) << "m.GiB/s" << std::setw(10) << "a.GiB/s" << std::setw(10) << "s.speed"
            << std::setw(10) << "e.speed" << '\n';
  for (const ResultRow &row : results) {
    std::cout << std::left << std::setw(4) << DirectionName(row.direction) << std::setw(7)
              ...
              << std::setw(11) << row.memcpy.submit_p50_us << std::setw(11) << row.memcpy.submit_p95_us
              << std::setw(11) << row.aicpu.submit_p50_us << std::setw(11) << row.aicpu.submit_p95_us
              << std::setw(11) << row.memcpy.e2e_p50_us << std::setw(11) << row.memcpy.e2e_p95_us
              << std::setw(11) << row.aicpu.e2e_p50_us << std::setw(11) << row.aicpu.e2e_p95_us
              << std::setw(10) << row.memcpy.gib_per_second << std::setw(10) << row.aicpu.gib_per_second
              << std::setw(10) << row.submit_speedup << std::setw(10) << row.e2e_speedup << '\n';
  }
}
...
void WriteCsv(const std::string &path, const std::vector<ResultRow> &results) {
  ...
  output << "direction,io_size_bytes,io_count,total_bytes,iterations,aicpu_kernel_launches,aicpu_notifies,"
...
int Run(int argc, char **argv) {
  ...
  if (!IsA3Soc(soc_name)) {
    throw std::runtime_error(std::string("AICPU direct RTSQ benchmark supports A3 only; detected SoC: ") +
                             (soc_name == nullptr ? "<unknown>" : soc_name));
  }
  std::cout << "device=" << options.device_id << " soc=" << soc_name
            << " kernel_json=" << options.kernel_json << '\n';
  std::cout << "Timing boundary: submit starts before descriptor allocation/build and ends after enqueue; "
  ...
  std::vector<ResultRow> results;
  {
    ...
    for (TransferDirection direction : options.directions) {
      for (uint64_t size : options.sizes) {
        for (size_t count : options.counts) {
          results.push_back(RunCase(memory, baseline, launcher, options, direction, size, count));
        }
      }
    }
  }
...
int main(int argc, char **argv) {
  try {
    ...
    std::cerr << "acl_malloc_copy_bench: " << error.what() << '\n';
    return 1;
  }
}
