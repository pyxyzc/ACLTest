...
MallocMemoryPair::~MallocMemoryPair() {
  Reset();
}

void MallocMemoryPair::Initialize(size_t bytes, int32_t logic_device_id) {
  if (bytes == 0U || logic_device_id < 0) {
    throw std::invalid_argument("malloc memory requires non-zero bytes and a non-negative device id");
  }
  ...
  try {
    ...
    bytes_ = bytes;
    std::cerr << "malloc memory: bytes=" << bytes_ << ", host=" << host_ << ", device=" << device_ << '\n';
  } catch (...) {
    Reset();
    throw;
  }
