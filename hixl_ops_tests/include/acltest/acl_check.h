...
public:
    ...
    aclError error() const noexcept { return error_; }
...
private:
    static std::string BuildMessage(const std::string &operation, aclError error)
    {
        ...
        if (message != nullptr && message[0] != '\0') { out << ", recent_error=" << message; }
        return out.str();
    }
...
{
    if (error != ACL_SUCCESS) { throw AclException(operation, error); }
}
