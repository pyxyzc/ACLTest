...
#ifndef ACLTEST_INCLUDE_ACLTEST_HIXL_OPS_LAUNCHER_H_
#define ACLTEST_INCLUDE_ACLTEST_HIXL_OPS_LAUNCHER_H_
...
class HixlOpsLauncher {
public:
    HixlOpsLauncher() = default;
    ~HixlOpsLauncher();
    HixlOpsLauncher(const HixlOpsLauncher &) = delete;
    HixlOpsLauncher &operator=(const HixlOpsLauncher &) = delete;
    ...
    void Submit(TransferDirection direction, const std::vector<AddressRange> &ranges,
                uint32_t timeout_ms);
    ...
    const SubmissionShape &last_shape() const { return last_shape_; }
...
private:
    ...
    void BuildRtsqParam(uint32_t task_count, HixlKernelParam &param);
    void LaunchBatches(TransferDirection direction, size_t descriptor_count, uint32_t timeout_ms);
    void LaunchOneBatch(aclrtFuncHandle function, TransferDirection direction, size_t begin,
...
#endif  // ACLTEST_INCLUDE_ACLTEST_HIXL_OPS_LAUNCHER_H_
