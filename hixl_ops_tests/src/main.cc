...
{
    return root + "/opp/built-in/op_impl/aicpu/config/libcann_hixl_kernel.json";
}
...
{
    const char *override_path = std::getenv("ACLTEST_HIXL_KERNEL_JSON");
...
void PrintUsage(const char *program)
{
    std::cout << "Usage: " << program << " [options]\n"
              ...
              << "  --kernel-json PATH      installed HIXL ops config JSON\n"
...
Timing RunHixlOps(HixlOpsLauncher &launcher, const std::vector<AddressRange> &ranges,
...
void VerifyMethods(FabricMemoryPair &memory, const std::vector<AddressRange> &ranges,
                   ...
                   BaselineStream &baseline, HixlOpsLauncher &launcher, uint32_t timeout_ms)
{
    ...
    if (direction == TransferDirection::kDeviceToHost) {
        ...
        std::memset(host, 0, total_bytes);
        (void)RunHixlOps(launcher, ranges, direction, timeout_ms);
        CheckEqual(host, pattern, "HIXL ops batch D2H");
        return;
    }
    ...
    ACLTEST_CHECK_ACL(aclrtMemcpy(actual.data(), total_bytes, memory.device(), total_bytes,
                                  ACL_MEMCPY_DEVICE_TO_HOST));
    CheckEqual(actual.data(), pattern, "HIXL ops batch H2D");
}
...
ResultRow RunCase(FabricMemoryPair &memory, BaselineStream &baseline, HixlOpsLauncher &launcher,
                  ...
                  size_t io_count)
{
    ...
    for (size_t index = 0U; index < options.warmup; ++index) {
        if ((index & 1U) == 0U) {
            ...
            (void)RunHixlOps(launcher, ranges, direction, options.timeout_ms);
        } else {
            (void)RunHixlOps(launcher, ranges, direction, options.timeout_ms);
            (void)RunMemcpyLoop(ranges, direction, baseline.get(), options.timeout_ms);
        }
    ...
    std::vector<Timing> hixl_ops_timings;
    memcpy_timings.reserve(iterations);
    hixl_ops_timings.reserve(iterations);
    for (size_t index = 0U; index < iterations; ++index) {
        if ((index & 1U) == 0U) {
            ...
            hixl_ops_timings.push_back(RunHixlOps(launcher, ranges, direction, options.timeout_ms));
        } else {
            hixl_ops_timings.push_back(RunHixlOps(launcher, ranges, direction, options.timeout_ms));
            memcpy_timings.push_back(
                RunMemcpyLoop(ranges, direction, baseline.get(), options.timeout_ms));
    ...
    result.hixl_ops = Summarize(hixl_ops_timings, total_bytes);
    result.submit_speedup = result.memcpy.submit_p50_us / result.hixl_ops.submit_p50_us;
    result.e2e_speedup = result.memcpy.e2e_p50_us / result.hixl_ops.e2e_p50_us;
    return result;
}
...
void PrintResults(const std::vector<ResultRow> &results)
{
    ...
    std::cout << std::left << std::setw(4) << "dir" << std::setw(7) << "size" << std::right
              ...
              << std::setw(11) << "h.s50" << std::setw(11) << "h.s95" << std::setw(11) << "m.e50"
              << std::setw(11) << "m.e95" << std::setw(11) << "h.e50" << std::setw(11) << "h.e95"
              << std::setw(10) << "m.GiB/s" << std::setw(10) << "h.GiB/s" << std::setw(10)
    ...
    for (const ResultRow &row : results) {
        std::cout << std::left << std::setw(4) << DirectionName(row.direction) << std::setw(7)
                  ...
                  << row.memcpy.submit_p95_us << std::setw(11) << row.hixl_ops.submit_p50_us
                  << std::setw(11) << row.hixl_ops.submit_p95_us << std::setw(11)
                  << row.memcpy.e2e_p50_us << std::setw(11) << row.memcpy.e2e_p95_us
                  << std::setw(11) << row.hixl_ops.e2e_p50_us << std::setw(11) << row.hixl_ops.e2e_p95_us
                  << std::setw(10) << row.memcpy.gib_per_second << std::setw(10)
                  << row.hixl_ops.gib_per_second << std::setw(10) << row.submit_speedup
...
void WriteCsv(const std::string &path, const std::vector<ResultRow> &results)
{
    ...
    output << "direction,io_size_bytes,io_count,total_bytes,iterations,hixl_ops_kernel_launches,hixl_ops_"
              "notifies,"
              "memcpy_submit_p50_us,memcpy_submit_p95_us,hixl_ops_submit_p50_us,hixl_ops_submit_p95_us,"
              "memcpy_e2e_p50_us,memcpy_e2e_p95_us,hixl_ops_e2e_p50_us,hixl_ops_e2e_p95_us,"
              "memcpy_gib_per_s,hixl_ops_gib_per_s,submit_speedup,e2e_speedup,verify\n";
    ...
    for (const ResultRow &row : results) {
        output << DirectionName(row.direction) << ',' << row.io_size << ',' << row.io_count << ','
               ...
               << row.memcpy.submit_p95_us << ',' << row.hixl_ops.submit_p50_us << ','
               << row.hixl_ops.submit_p95_us << ',' << row.memcpy.e2e_p50_us << ','
               << row.memcpy.e2e_p95_us << ',' << row.hixl_ops.e2e_p50_us << ','
               << row.hixl_ops.e2e_p95_us << ',' << row.memcpy.gib_per_second << ','
               << row.hixl_ops.gib_per_second << ',' << row.submit_speedup << ',' << row.e2e_speedup
               << ",PASS\n";
    }
...
int Run(int argc, char **argv)
{
    ...
    if (!IsSupportedDirectRtsqSoc(soc_name)) {
        throw std::runtime_error(
            std::string("HIXL ops direct RTSQ benchmark supports A3/A5 only; detected SoC: ") +
...
int main(int argc, char **argv)
{
    try {
        ...
        std::cerr << "hixl_ops_bench: " << error.what() << '\n';
        return 1;
    }
}
